...
Store KEF-encrypted seed backups on NFC cards through an external WS1850S module (M5Stack RFID Unit 2) on I2C. The card is a third destination alongside flash and SD: the same envelope, the same password prompt, a different medium. MIFARE Classic 1K/4K and Ultralight/NTAG21x are supported. The on-card format is shared with the Kern NFC branch, which originated it, so a card written by either firmware reads on the other.
