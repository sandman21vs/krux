...
Store KEF-encrypted seed backups on NFC cards through an external reader. The card is a third destination alongside flash and SD: the same envelope, the same password prompt, a different medium. Two readers are supported, a WS1850S (M5Stack RFID Unit 2) on I2C and a PN5180 on SPI, selected under `Settings > Hardware > NFC > Reader`. MIFARE Classic 1K is the only tag family supported: it is the one that has been through a card, and its capacity is known rather than read from the tag. The on-card format is shared with the Kern NFC branch, which originated it, so a card written by either firmware reads on the other.
